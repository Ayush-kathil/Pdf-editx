...
export default function MovToMp4Page() {
  ...
  const load = async () => {
    ...
    try {
        ...
        setError("Failed to load video engine. Please try reloading (Multi-thread support required).");
    }
  };
  ...
  const handleDownload = (file?: {name: string, blob: Blob}) => {
    if (file) {
        ...
        // Download all - handled individually for simplicity in code, usually zip for batch
        // Implementing simple loop for "Download All"
